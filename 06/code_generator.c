...
{
    printf("gen_program");
    printf("name : %d,%d value : %d", N_PROGRAM, node->name, node->value);
    switch(node->name)
    {
        case N_PROGRAM: 
...
void gen_declaration(A_ID * id){
    ...
    switch(id->kind){
        case ID_VAR : 
            // if (id->init)
            //     if(id->level == 0)
            //         gen_error(111, id->line, "GLOBAL INIT");
            //         // gen_initializer_global(id->init, id->type, id->address);
            //     else
            //         gen_error(111, id->line, "LOCAL INIT");
        ...
        case ID_STRUCT :
        case ID_FIELD :
            gen_error(111, id->line, "Struct and Field");
            break;
...
void gen_statement(A_NODE * node, int cont_label, int break_label, A_SWITCH sw[], int * sn){
    ...
    switch(node->name){
        case N_STMT_LABEL_CASE :
            ...
            gen_error(111, node->line, "case Label");
            break;
        case N_STMT_LABEL_DEFAULT:
            ...
            gen_error(111, node->line, "default Label");
            break;
        ...
        case N_STMT_IF_ELSE:
            ...
            break;
        case N_STMT_SWITCH :
            gen_error(111, node->line, "SWITCH");
            // gen_expression(node->llink);
...
void gen_expression(A_NODE *node)
{
    ...
    switch (node->name){
            ...
            switch(id->kind){
                ...
                case ID_PARM :
                    switch(t->kind){
                        ...
                        case T_STRUCT:
                        case T_UNION:
                            gen_error(111, id->line, "Struct and Union");
                            // gen_code_i(LDA, id->level, id->address);
                            // i = id->type->size;
        ...
        case N_EXP_STRUCT : 
        case N_EXP_ARROW : 
            gen_error(111, id->line, "STRUCT OR ARROW");
            break;
        case N_EXP_POST_INC :
        ...
        case N_EXP_ADD:
            gen_expression(node->llink);
            if (isPointerOrArrayType(node->rlink->type)){   //Array type L?
                gen_code_i(LITI, 0, node->rlink->type->element_type->size);
                gen_code_i(MULI, 0, 0);
            }
            gen_expression(node->rlink);
            if (isPointerOrArrayType(node->llink->type)){   //Array Type R?
...
void gen_initializer_global(A_NODE *node, A_TYPE * t, int a){
    ;
}
void gen_initializer_local(A_NODE *node, A_TYPE * t, int a){
    ;
}
...
void gen_error(int i, int ll, char * name){
    gen_err++;
    ...
    switch(i){
        ...
        case 23 : 
            ...
            break;
        case 100 : 
            printf("fatal compiler error durind code generation\n");
            break;
        case 111 :
            printf("No implementation %s\n", name);
            break; 
        default : 
